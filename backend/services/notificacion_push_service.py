...
class NotificacionPushService:
    ...
    def enviar(self, usuario_id: UUID, titulo: str, cuerpo: str, url: str = None):
        ...
        for suscripcion in suscripciones:
            ejecutar_en_segundo_plano(self._enviar_una, suscripcion, titulo, cuerpo, url)

    def _enviar_una(self, suscripcion, titulo: str, cuerpo: str, url: str = None):
        try:
            subscription_info = {
            ...
            if ex.response is not None and ex.response.status_code in (410, 404):
                ...
                logger.error(f"Error enviando push a {suscripcion.endpoint}: {ex}", exc_info=True)
                sentry_sdk.capture_exception(ex)
        except Exception as ex:
            logger.error(f"Error inesperado enviando push a {suscripcion.endpoint}: {ex}", exc_info=True)
            sentry_sdk.capture_exception(ex)